...
proc transitionImageLayout(
  ...
  var
    sourceStage, destinationStage: VkPipelineStageFlags2
...
proc uploadTexture(ctx: VulkanContext, renderer: var SpriteRenderer) =
  ...
  transitionImageLayout(ctx, renderer.textureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL, renderer.textureMipLevels)
...
proc createGraphicsPipeline(ctx: VulkanContext, renderer: var SpriteRenderer) =
  ...
  try:
    var
      ...
      viewportState = VkPipelineViewportStateCreateInfo(sType: VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO, viewportCount: 1, scissorCount: 1)
...
proc endDraw(
  ...
  var
    clearValue = VkClearValue(color: VkClearColorValue(float32: clearColor))
...
proc drawFrame(ctx: var VulkanContext, renderer: var SpriteRenderer, drawer: SpriteDrawer, clearColor: array[4, float32]): bool =
  ...
  checkVk(
    ...
    "Submitting sprite command buffer"
  )
